...
export default defineEventHandler(async (event) => {
  ...
  // Determine category & preset attributes
  let category = 'general'
  if (pLower.includes('ai') || pLower.includes('aigc') || pLower.includes('sora') || pLower.includes('runway') || pLower.includes('comfyui') || pLower.includes('生成视频') || pLower.includes('扩散')) {
    category = 'ai_video'
  ...
  // Generate Title & Slug
  let cleanName = prompt.replace(/(商业广告|概念片|宣传片|短片|MV|视频|生成|AI|纪录片|TVC)/g, '').trim()
  if (!cleanName) cleanName = prompt

  let title = `${cleanName} · 概念作品`
  let slug = prompt.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '')
  ...
  switch (category) {
    case 'ai_video':
      title = `${cleanName} · AIGC 扩散生成短片`
      description = '融合 Sora / Diffusion 扩散模型与达芬奇 DI 色彩科学，以神经网络高维 Latent 空间编织未来光影景象。'
...
### 02. DI 达芬奇超分重构与调色 (Color Reconstruction)
将神经网络输出的低分辨视频引入 DaVinci Resolve 19 达芬奇 Neural Engine，执行 4K 超分降噪与色彩管理。匹配 ACES 1.3 工业级宽容度，将 AIGC 的数字冷感转化为古典电影级的细腻胶片层次。

### 03. 声音设计与空间音效合成 (Audio Synthesis)
采用 24-bit 96kHz 无损三维沉浸式声场，将电子合成器低频律动与拟音细节融合，赋予画面前所未有的科幻沉浸感与情绪冲击。`
      workflow = [
        { phase: 'Phase 01', title: 'Multi-Model Prompt 提示词分镜卡位', desc: '蒸馏核心视觉提示词，建立控制网与种子节点，输出初始高质感流体动效。' },
        { phase: 'Phase 02', title: 'Neural Latent Motion 神经扩散与插帧', desc: '结合 Runway 与 Topaz Video AI 执行神经插帧与无损放大，消除 AI 抖动与伪影。' },
    ...
    case 'automotive':
      ...
      description = '以变形宽银幕电影镜头语言结合暗金调色科学，锤炼极致金属光泽与机械力学与速度美学。'
      postSpecs = '4K 60FPS Anamorphic'
      deliverFormat = 'ARRI LogC4 to Rec.709'
      audioFormat = '24-bit 96kHz 5.1 Surround'
      software = ['DaVinci Resolve', 'Premiere Pro', 'Cinema 4D', 'Redshift']
      tags = ['汽车广告', '变形宽银幕', '暗金调色', '高光细节', '机械力学']
      longDescription = `### 01. 光影与金属车漆质感 (Color & Specular Control)
基于 ARRI LogC4 宽容度曲线，在 DaVinci DI 色彩实验室中构建专属的“玄武暗金”色调。针对车身哑光金属漆与流线型高光执行单色相精细分离，呈现沉稳奢华的视觉光辉。

### 02. 速度感卡点与镜剪节奏 (Editing Mechanics)
利用气缸爆震与引擎轰鸣的真实声轨峰值，进行毫秒级精准卡点剪辑。结合追车镜头摇移与微距镜头切割，展现车身极速跃进时的律动美感。

### 03. 三维高精光效与 CGI 合成 (3D & VFX Motion)
结合 Cinema 4D 与 Redshift 渲染器合成高质感粒子烟尘与空气动力学流线，赋予影片顶尖车企公映大片的震撼质感。`
      workflow = [
        { phase: 'Phase 01', title: 'Offline 粗剪与引擎声轨精准卡点', desc: '根据发动机震浪与音效波形卡位，奠定震撼利落的剪辑基调。' },
        { phase: 'Phase 02', title: 'DI 达芬奇暗金色调与漆面光泽重构', desc: '压制高光反射，分离车漆单色相，打造极致暗奢的玄武金质感。' },
        { phase: 'Phase 03', title: '5.1 环绕声混音与 4K ProRes 4444 交付', desc: '整合 V8 引擎原声采样与母带压限，完成全球公映级 Master 导出。' }
      ]
      break

    case 'commercial':
      title = `${cleanName} · TVC 商业概念广告`
      description = '融合大刊级静物拍摄、极致卡点节奏与色彩科学，为品牌塑造高奢 Quiet Luxury 现代美学。'
      ...
      tags = ['商业广告', 'TVC', '卡点剪辑', '高光细节', '品牌大片']
      longDescription = `### 01. 商业级品牌调性塑造 (Brand Aesthetics)
遵循现代极奢设计理念，在镜头编排上融合高帧率慢动作与快节奏镜头拼接。以静衬动，打造具有呼吸感的大刊封面视觉体验。

### 02. DI 达芬奇色彩管理 (DI Color Management)
在 Rec.709 与 HDR 两种规范间精细校准，针对产品材质、肤色透亮感进行高精度 Mask 蒙版分离，使画面高光柔润而不暴晒，暗部沉静而不失细节。

### 03. 动效包装与声音强化 (Motion & SFX)
嵌入无缝光影过渡与极简三维文字包装，配合沉浸式音效层次强化品牌核心卖点，打造顶尖商业标杆。`
      workflow = [
        { Phase: 'Phase 01', title: 'Offline 粗剪与品牌叙事搭骨架', desc: '梳理产品核心卖点，按商业 TVC 黄金节奏搭建镜头框架。' },
    ...
    case 'film':
      ...
      description = '使用 DCI 4K 胶片质感与 ACES 1.3 工业色彩空间，通过镜头调度与暗部解调雕琢叙事张力。'
      postSpecs = 'DCI 4K 24FPS'
      deliverFormat = 'ProRes RAW / ACES 1.3'
      audioFormat = '24-bit 96kHz 5.1 Surround'
      software = ['DaVinci Resolve', 'Premiere Pro', 'Logic Pro']
      tags = ['电影短片', '胶片复古', 'ACES1.3', '暗部细节', '剧情叙事']
      longDescription = `### 01. 胶片模拟与 ACES 1.3 流程 (Film Print Emulation)
采用 ACES 1.3 工业级色彩架构，精准印片模拟 Kodak 2383 胶片曲线。在保留影调肉感的同时，注入自然润泽的胶片银盐颗粒。

### 02. 叙事剪辑与情感呼吸感 (Narrative Pacing)
拒绝堆砌无意义特效，强调视线引导与角色心理状态的镜像剪辑。以留白与呼吸停顿展现沉稳而深刻的电影感。

### 03. 原声管弦配乐与声场设计 (Orchestral Score & Foley)
结合低音大提琴沉压与实录 Foley 声音，构建空间维度极其深邃的环境声场，增强戏剧沉浸感。`
      workflow = [
        { phase: 'Phase 01', title: 'Offline 叙事剪辑与戏剧张力梳理', desc: '专注于角色心理与动作线剪辑，剔除赘余镜头，奠定电影语感。' },
    ...
    case 'fashion_mv':
      ...
      description = '以极简冷调与富贵高光塑造前卫视觉体验，结合激进剪辑跳切与先锋音乐卡点。'
      ...
      tags = ['时尚MV', '极简冷调', '跳切卡点', '先锋美学', '高光透亮']
      longDescription = `### 01. 时尚冷调美学 (Minimalist Cool Tone)
运用 ARRI LogC4 广色域，打磨具有冷冽高贵感的中性灰调。重点提升肤色透明感与发丝高光，呈现高定秀场般的高级沉静感。

### 02. 视听跳切与声画异步 (Match Cut & Jump Cut)
结合前卫音乐节奏采用大幅度跳切与色彩突变，通过画面错位与声画异步营造强烈的时尚视觉冲击。

### 03. 动态视觉符号与质感滤镜 (Optical Motion)
叠加变形镜头光斑、晕化高光滤镜与微粒子光效，赋予 MV 浓郁的现代艺术气息。`
      workflow = [
        { phase: 'Phase 01', title: 'Offline 快节奏跳切与先锋卡位', desc: '破坏常规叙事，以音乐强弱拍与色彩闪烁进行前卫片段堆叠。' },
    ...
    case 'tech':
      ...
      description = '渲染极致工业设计之美，结合 8K 60FPS 超清规格与微米级 CGI 三维解构。'
      postSpecs = '8K 60FPS Ultra HD'
      deliverFormat = 'ProRes 4444 XQ / Rec.2020 HDR10'
      audioFormat = '24-bit 96kHz Uncompressed'
      software = ['Cinema 4D', 'Redshift', 'After Effects', 'DaVinci Resolve']
      tags = ['科技发布会', '8K超清', 'CGI三维', '工业设计', 'HDR10']
      longDescription = `### 01. 8K 超清细节与极致材质 (Ultra HD & Materials)
在 8K Rec.2020 广色域下呈现数码硬件微米级玻璃、阳极氧化铝与精密电路板。画质清晰锐利，光影过渡绵密真实。

### 02. 三维爆炸拆解与科技光影 (3D Explosion & VFX)
通过 Cinema 4D 动态粒子与 Raytracing 光线追踪，生动演绎内部架构与芯片计算能效。

### 03. 未来感声效与发布会大屏优化 (Tech SFX & Large Screen)
定制机械转动、高频脉冲与沉浸式低音，确保影片在大型发布会公映大屏上展现强悍的视听震撼。`
      workflow = [
        { phase: 'Phase 01', title: '3D 分镜预演与镜头轨迹匹配', desc: '建立硬件 CAD 模型，搭建精确摄像机运动轨道与推拉光影。' },
    ...
    case 'cyberpunk_vfx':
      ...
      description = '高帧率慢动作与 CG 特效合成，打造霓虹雨夜、暗影光芒与未来赛博美学。'
      postSpecs = 'DCI 4K 120FPS High-Frame'
      deliverFormat = 'OpenEXR / ACES 1.3 CG'
      audioFormat = '24-bit 96kHz 7.1 Surround'
      software = ['Cinema 4D', 'Octane', 'After Effects', 'DaVinci Resolve']
      tags = ['赛博朋克', 'VFX特效', '120FPS', 'OpenEXR', '科幻视觉']
      longDescription = `### 01. 霓虹暗影与 ACES CG 色彩 (Neon & ACES CG)
以紫青冷暖色相对比为核心，在 ACES CG 空间中混炼高动态范围霓虹辉光。湿滑路面反射与粒子全息投影错落有致。
...
融合工业重金属与合成器浪潮，搭配电波干扰声，构建宏大而冰冷的科幻世界观。`
    ...
    case 'documentary':
      ...
      description = '真实自然色彩与柯达胶片发色，用温暖克制的光影记录最打动人心的人文温度。'
      postSpecs = '4K 24FPS Film-Grain'
      deliverFormat = 'Kodak 2383 Print Emulation'
      audioFormat = '24-bit 48kHz Stereo'
      software = ['DaVinci Resolve', 'Premiere Pro']
      tags = ['人文纪录片', '自然色彩', '柯达胶片', '情绪叙事', '真实质感']
      longDescription = `### 01. 真实自然的胶片色彩 (Natural Film Color)
拒绝过度包饰，模拟 Kodak 自然暖质曲线。保留真实自然日光与室内温润烛光的影调层次，凸显人文温度。
...
融入木吉他弹拨与自然风声、雨声，呈现极具真实质感的人文纪录片沉浸感受。`
      workflow = [
        { phase: 'Phase 01', title: '故事线梳理与生活化镜头留白', desc: '遵循真实素材逻辑，剪辑人物对话与环境镜头，营造留白情绪。' },
        { phase: 'Phase 02', title: 'DI 达芬奇自然柯达印片模拟', desc: '调校温暖真实的自然肤色，保留环境天然影调与微细颗粒。' },
    ...
    case 'live_concert':
      ...
      description = '多机位精准同步，配合灯光强频闪与台下咆哮声轨，呈现荷尔蒙爆棚的 Live 现场。'
      postSpecs = '4K 60FPS Multi-Cam'
      deliverFormat = 'ProRes 422 HQ / Live Master'
      audioFormat = '24-bit 96kHz Multi-Track Audio'
      software = ['Premiere Pro', 'DaVinci Resolve', 'Logic Pro']
      tags = ['Live现场', '音乐节', '多机位', '强律动', '爆棚荷尔蒙']
      longDescription = `### 01. 多机位时间码同步 (Multi-Cam Sync)
通过声音波形与时间码同步 12 路实拍机位，涵盖舞台全景、乐手特写与观众席人浪。
...
混响现场歌迷合唱与主唱声音，消除现场噪音，呈现媲美现场亲临感的高保真音质。`
      workflow = [
        { phase: 'Phase 01', title: '12 机位多轨同步与重音快剪', desc: '对齐所有机位时间码，配合音乐强拍进行毫秒级镜头对切。' },
    ...
    default:
      description = `针对 ${prompt} 打造的 Quiet Luxury 高奢剪辑与达芬奇 DI 色彩解构作品。`
      postSpecs = '4K 60FPS HDR'
      deliverFormat = 'ProRes 4444 XQ / Rec.709'
      audioFormat = '24-bit 48kHz'
      software = ['DaVinci Resolve', 'Premiere Pro', 'After Effects']
      tags = ['剪辑节奏', '达芬奇调色', '高光细节', '商业级交付']
      longDescription = `### 01. 镜头语言与节奏编排\n基于 ${prompt} 的核心表达，梳理高帧率镜头与故事主线的切分卡位。\n\n### 02. DI 达芬奇色彩科学\n采用工业级色彩管理，呈现通透自然的暗部细节与柔润高光。\n\n### 03. 声音设计与交付规格\n匹配无损声轨与 ProRes 商业公映级文件，确保全平台展现顶级视听质感。`
      workflow = [
        { phase: 'Phase 01', title: 'Offline 粗剪与节奏搭建', desc: '根据背景声轨与击鼓声的峰值波形进行精确画面切割与卡位。' },
