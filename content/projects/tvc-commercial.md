---
title: "2026 个人剪辑与后期样片秀 (Showreel)"
description: "剪辑节奏 · 电影感调色 (DaVinci) · 三维动效 (C4D)"
longDescription: "本样片秀汇集了近一年来制作的旗舰级 TVC 商业广告、高奢概念片以及运动特效镜头的混剪。运用了极致节奏的声轨切割与三维动效包装，呈现了影视后期的工业级精剪工作流。"
image: "https://images.unsplash.com/photo-1542204172-e7052809f852?w=800&q=80"
videoUrl: "https://assets.mixkit.co/videos/preview/mixkit-cyberpunk-neon-sign-with-a-retro-style-41578-large.mp4"
password: ""
imageBefore: "https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=800&q=80"
tags:
  - "剪辑节奏"
  - "电影感调色"
  - "三维动效"
software:
  - "Premiere Pro"
  - "DaVinci Resolve"
  - "After Effects"
  - "Cinema 4D"
featured: true
workflow:
  - icon: "⚡"
    title: "Offline 粗剪"
    desc: "根据背景声轨与击鼓声的峰值波形进行精确画面切割与卡位。"
  - icon: "🎨"
    title: "色彩分级"
    desc: "在 DaVinci Resolve 中转换 Arri LogC 视频，建立 ACES 色彩空间，渲染哑金古铜色调。"
---

这里是作品的详细描述内容。您可以在此编辑和查看。
